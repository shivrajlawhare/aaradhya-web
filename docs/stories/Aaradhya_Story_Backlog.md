...
**Decisions (v1):**
- **No later story adds an entry point into this screen** (checked STORY-046/047, the next two stories in the backlog — neither mentions it), so this story adds one itself: a "Preview Quotation" link in `overview-tab.tsx`, next to STORY-044's own "Generate Quotation PDF" button, EventManager-gated the same way.
- New route `QUOTATION_PREVIEW_PATH_PATTERN = '/events/:id/quotation-preview'` registered in `app.tsx` with **no `RequireRole` wrapper** — every field this screen shows (Client Details, Sessions, Accommodation, the Total Cost Summary rollup) is already visible to any authenticated caller via the Overview/Rooms/Sessions tabs and `GET /events/:id/quotation-summary` itself (STORY-041's own "any authenticated caller" call), so the whole page isn't a new information leak. Only the **Share PDF action** inside it is Event-Manager-gated, since generating the PDF calls the Event-Manager-only STORY-043 endpoint and would just 403 for anyone else.
- **Reuses `TotalCostSummaryPanel` and `GenerateQuotationPdfButton` directly**, not reimplemented — the AC's own wording ("reusing STORY-042's panel," "triggers the same flow as STORY-044") is read literally. `GenerateQuotationPdfButton` gained one new optional `label?: string` prop (defaulting to its existing "Generate Quotation PDF" text) so this screen can render it as "Share PDF" without a second implementation of the download/loading/error logic.
- Rendered with `canEdit={false}` unconditionally on `TotalCostSummaryPanel` — this is a read-only preview to sanity-check numbers before sharing, not another place to edit extras; that already exists on the Overview tab.
- A Cancelled Session is excluded from the per-session list here, matching aaradhya-api's own STORY-043 PDF renderer's own exclusion exactly (and STORY-041's underlying filter) — showing it as a real scheduled item on a screen whose whole point is "mirrors the same data [as the PDF]" would visually contradict the Grand Total directly below it, which already excludes that Session's cost.
- **"Crest/wordmark" is rendered as styled text** ("Aaradhya" in `variant="display"`), not an image — no crest/logo asset exists anywhere in this repo (`public/`, `src/assets/`, or otherwise), and `docs/design/theme-tokens.md`'s own Fraunces role has always meant styled text via the `display` Typography variant (confirmed against `total-cost-summary-panel.tsx`'s own Grand Total, STORY-042), never an image.
- **Cross-check against STORY-043 (AC-3) is verified by a test that fetches both the mocked `quotation-summary` and `quotation.pdf` endpoints for the same fixture Event and asserts the numbers are numerically identical** — not by generating/parsing a real PDF binary inside this repo. The mocked PDF response body is plain text embedding the same computed grand total (not real PDF bytes via `pdfkit`), since aaradhya-api's own STORY-043 test suite already covers real PDF generation/text-extraction; duplicating that tooling into this frontend-only repo would test theater, not any behavior unique to this story. The screen displays the grouped/formatted number (`formatAmount`, en-IN) while the real PDF renderer interpolates the raw number with no formatting (`aaradhya-api`'s own `quotation-pdf.ts`) — "matches... exactly" is read as numeric equality, not textual/formatting equality, and the test asserts both explicitly.
...
**Decisions (v1):**
- **One shared `DashboardPage` for all four roles, not an Event-Manager-specific component with a separate wrapper per role.** `GET /dashboard` (STORY-047) already varies its own response by `req.user.role` server-side (STORY-046's `filterEventForRole`), and the `/dashboard` route itself carries no `RequireRole` (matching `GET /events`/`GET /calendar`'s own precedent) — so the exact same component already renders correctly for whichever role is logged in, using whatever fields the API actually returned. This *is* "STORY-048's layout component" that STORY-049/050/051 each say they "reuse" — those stories don't need a new component, only verification against real per-role fixture data (plus F&B's own decided-and-documented extra column, per STORY-049's own AC wording).
- **`type-title-l`, not `type-display`, for the count numbers** — the story's own Tokens line offers either. Fraunces has been consistently scoped to a few specific "the app is speaking as Aaradhya" moments (wordmark, Quotation header, the Grand Total on the Total Cost Summary panel, STORY-042/045) — a functional dashboard count isn't one of those, so `titleL` (Inter) was chosen to keep that scoping narrow rather than diluting it.
- **"SRS §6 mock layout"** (this story's own AC) **doesn't exist** — checked before implementing (same discrepancy already flagged for STORY-045's identical citation): the SRS's actual §6 is Non-Functional Requirements, and no dashboard mock/wireframe exists anywhere in either repo's `docs/`. The AC's own prose (six named columns) is the only layout spec that actually exists, and is what was built.
- **Row click-to-navigate, though not explicitly named in this story's AC**, was added to the upcoming-events table — matching `EventsTable`'s own already-established convention for event-listing tables in this app, and consistent with the Flow's own framing (a dashboard's whole point is getting to an Event). This surfaced a real second occurrence of the exact same row-activation logic, extracted into `src/components/ui/event-row-activation.ts` and adopted by both tables (a plain function, not a hook, since it's called once per row inside `.map()`).
- **The "client" column shows a comma-joined list of names, or "—"** when `clientContacts` is absent entirely for a role (e.g. Housekeeping, per STORY-046) — not blank, not an error, and genuinely reading "the key is absent" rather than "the array happens to be empty," matching STORY-046's own "genuinely absent, not null" convention end-to-end into the UI.
- **`dashboard-placeholder/` deleted outright**, not kept alongside the real page — STORY-004's own placeholder-routing gap is this story's explicit job to close, and nothing else in the app referenced that folder (confirmed via grep before removing).
...
**Decisions (v1):**
- **The AC's own "seven tabs" (Overview, Client, Sessions & Menu, Setup, Rooms, Payments, Documents) don't map onto seven distinct built tabs.** Client Contacts has lived inside Overview since STORY-017; Setup and Menu/Items have lived inside the Sessions tab (per-session, inside the Session form) since STORY-026/STORY-033. Neither story ever built a standalone Client or Setup tab, and this story's own UI line ("no new visual surface") rules out inventing them now. The AC's per-role bullets are read as restating SRS §3.x's field-visibility table in tab-name shorthand, mapped onto the six tabs that actually exist (Overview, Rooms, Sessions, Payments, Documents, Activity — Activity itself isn't named in FR-ROLE-4's own seven-tab list either, and stays Event-Manager-only, unchanged).
- **Resulting tab-visibility matrix**, derived directly from the AC's own three per-role bullets: EventManager — all six tabs, unchanged. F&B Head — Overview, Sessions (no Rooms/Payments/Documents/Activity). Housekeeping — Overview, Rooms, Sessions (no Payments/Documents/Activity). Reception — Overview, Rooms (no Sessions at all — the AC's own "Payments and Sessions & Menu are absent" for Reception is read literally as the whole tab, unlike Housekeeping's own "Sessions & Menu (the menu part specifically)" wording, which only excludes Menu within a Sessions tab Housekeeping does see, for Setup's sake).
- **Within the Sessions tab, Setup and Menu are further gated per row**, not per-tab: Housekeeping sees a "Setup: …" summary line but no "Menu: …" line; F&B Head sees "Menu: …" but no "Setup: …". Neither summary line is added to the **Event Manager's own** list view — this story's own regression AC ("Event Manager's view is unchanged") takes priority over extending the summary to everyone able to see the raw data; Event Manager already reaches full Setup/Item detail one click away via "Edit".
- **A real, pre-existing gap this story had to fix to make its own ACs possible at all: aaradhya-web's own `eventResultSchema` never caught up with aaradhya-api's STORY-046 `filteredEventResultSchema`.** `GET /events/:id` has returned role-filtered data (clientContacts/accommodation/payment/extras/session.venueCost/session.setup/session.items all `.optional()`, genuinely absent per role) since STORY-046 landed — but until this story, only an Event Manager had ever actually opened Event Detail (no other role's tabs existed yet), so nothing forced the frontend's own contract mirror or any component to handle the optional case. Several components would have crashed the instant a non-EventManager session reached them: `OverviewTab`'s read-only Client Contacts branch (`event.clientContacts.map` on `undefined` for Housekeeping), `TotalCostSummaryPanel`'s `useForm` defaultValues (`event.extras` undefined for everyone but Event Manager), `RoomsTab`'s editable-branch defaultValues (`event.accommodation` undefined for F&B Head). Added `filteredItemResultSchema`/`filteredSessionResultSchema`/`filteredEventResultSchema` (and exported `filteredRoomLineResultSchema`/`filteredAccommodationResultSchema`, generalized from STORY-050's own dashboard-local versions) mirroring the backend's own names exactly, and switched `getEvent`'s contract entry to use it.
- **`TotalCostSummaryPanel` (Overview) is now gated to Event Manager only**, previously rendered unconditionally to everyone. Its content (Grand Total, extras, cost breakdown) is exactly the class of financial data STORY-046 already strips end-to-end for every other role (`payment`/`extras` are the two fields that story's own filter always omits, full stop, no role gets a partial view) — this panel just never crashed on that gap before because no other role could reach the tab. Same reasoning for `PaymentsTab`, unchanged (already Event-Manager-only), and for the Quotation Preview screen below.
- **The Quotation Preview route (`/events/:id/quotation-preview`) is now `RequireRole([EventManager])`-gated**, not left open. Its own prior "no RequireRole" comment reasoned that every field it shows was "already visible to any authenticated caller via the Overview/Rooms/Sessions tabs" — true only because those tabs weren't role-filtered yet. Leaving this screen open after this story's own gating landed everywhere else would have been a direct, easily-reachable bypass of the very restriction this story exists to add (it renders the *exact same* Total Cost Summary/Grand Total/full session-cost breakdown the Overview tab now hides). `canShare`/`useAuth` removed from the page itself — `RequireRole` already guarantees every session reaching this component is an Event Manager, matching `EventCreationPage`/`UserManagementPage`'s own established pattern of relying on the route guard alone, with no redundant internal role check. No new test re-verifies this route's own redirect specifically — `RequireRole`'s generic redirect behavior is already covered once, generically, in `require-role.test.tsx`, and no sibling EventManager-only page duplicates that coverage per-page either.
- **`formatSetup` extracted** to `src/components/ui/format-setup.ts` — the dashboard's own STORY-050 formatter and this story's new Sessions-tab Setup summary both operate on the exact same `sessionSetupResultSchema` shape; a genuine second caller for identical logic, not a forced abstraction (the F&B Head "Menu" summary was **not** extracted the same way — it reads the full `itemResultSchema` shape and needs its own Meal-type filter, a materially different job from the dashboard's own pre-trimmed, pre-filtered `dashboardUpcomingMealResultSchema`).
- **A real, independent display bug in `RoomsTab`'s read-only view, caught while touching this code**: `totalInclGst`/`totalCharges` were interpolated directly with no fallback, which would have rendered the literal text "undefined" the first time a role other than Event Manager (whose own accommodation always has both, unfiltered) opened the tab — fixed with `?? '—'`, consistent with this codebase's own "genuinely absent, not null" display convention everywhere else.
- **Three "own named flag" additions follow the same precedent STORY-017 already set for `canSeeActivity`**: `canSeeClientContacts` (Overview), `canSeeRooms`/`canSeeSessions` (the tab strip) are each explicit role-equality checks in `event-detail-page.tsx`, not derived from `event.clientContacts`/`event.accommodation` presence alone — kept as separate, explicit business rules even where they coincide with what the data happens to contain, matching this file's own stated reasoning for keeping `canSeePayments`/`canSeeDocuments` distinct from `canSeeActivity` despite evaluating the same way today.
- **The three STORY-010/023/044 flagged re-checks all held**: STORY-010/017's Activity gating, STORY-023's Payments DOM-absence, and STORY-044's PDF-button `canEdit` inheritance were all already correctly implemented when built; this story added no fix for any of them, only confirmed each still passes (regression tests already existed or were extended, not introduced from scratch).
